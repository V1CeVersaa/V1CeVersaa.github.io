...
所以我们将实例可以映射成字母表为 $\Sigma = \{0, 1\}$ 的字符串。

对于算法 $A$ 和形式语言的关系，我们有下面的内容：

- 对于一个字符串 $x\in \{0, 1\}^*$，如果 $A(x) = 1$，就称 $A$ **接受**这个字符串 $x$；
...
$$
\mathsf{P} = \{ L \subseteq \{0, 1\}^* : \text{there exists a polynomial-time algorithm } A \text{ that decides } L \}.
$$
...
有了计算模型之后，我们可以定义复杂度类了。我们考了一族函数 $f(n)$，称一个问题是：
...
注意到 co-$\mathsf{C}$ 并不意味着在所有问题意义上的补，而是其中所有语言的补。

**定理**：$\mathsf{P} = \text{co-}\mathsf{P}$。证明显然，只需要将这个图灵机的接受和拒绝调换就可以。
